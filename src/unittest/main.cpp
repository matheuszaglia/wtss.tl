...
{
  wtss::cxx::client remote("http://www.dpi.inpe.br/ts/wtss");
  std::vector<std::string> coverages = remote.list_coverages();
  te::st::TimeSeries* ts = new te::st::TimeSeries(coverages.front());

  wtss::cxx::geoarray_t cv = remote.describe_coverage(coverages.front());
  ...
  wtss::cxx::timeseries_query_result_t result = remote.time_series(q);
  ...
  te::qt::widgets::TimeSeriesChart* timeseries =
      new te::qt::widgets::TimeSeriesChart(ts);
  ...
  te::qt::widgets::ChartDisplay* chartDisplay =
      new te::qt::widgets::ChartDisplay(0, "Time Series", chartStyle);
  chartDisplay->adjustDisplay();
  timeseries->attach(chartDisplay);

  te::qt::widgets::ChartDisplayWidget* displayWidget =
      new te::qt::widgets::ChartDisplayWidget(
          timeseries, te::qt::widgets::TIMESERIES_CHART, chartDisplay);
  displayWidget->show();
